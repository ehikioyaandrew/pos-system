...
function linesTable(title: string, lines: ReportLine[], total: number, accent: string) {
  const rows =
    lines.length === 0
...
export function buildSalesReportHtml(p: SalesEmailPayload): string {
  const color = p.primaryColor && /^#?[0-9a-fA-F]{3,8}$/.test(p.primaryColor)
  ...
  const reminder = p.reminder
    ? `<div style="background:#fff7ed;border:1px solid #fed7aa;padding:14px 16px;border-radius:8px;margin:0 0 20px;">
        ...
        <p style="margin:6px 0 0;font-size:13px;color:#9a3412;">Open POS on the shop PC → <strong>Sync now</strong> so yesterday’s offline sales are in this report. Then you can rely on the numbers below.</p>
      </div>`
    : ''
