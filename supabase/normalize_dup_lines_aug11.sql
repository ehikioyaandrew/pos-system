-- =============================================================================
-- 11 Aug 2026 — same product listed twice on one sale (staff switched
...
-- -----------------------------------------------------------------------------
-- A) PREVIEW — duplicate lines on 11 Aug (Africa/Lagos)
-- -----------------------------------------------------------------------------
WITH bounds AS (
  SELECT
    (DATE '2026-08-11'::timestamp AT TIME ZONE 'Africa/Lagos') AS start_at,
    ((DATE '2026-08-11' + 1)::timestamp AT TIME ZONE 'Africa/Lagos') AS end_at
),
day_sales AS (
...
WHERE e.sale_id = s.id
  ...
  AND s.id IN (
    ...
    WHERE (NULLIF(TRIM(s2.created_at::text), ''))::timestamptz
            >= (DATE '2026-08-11'::timestamp AT TIME ZONE 'Africa/Lagos')
      AND (NULLIF(TRIM(s2.created_at::text), ''))::timestamptz
            < ((DATE '2026-08-11' + 1)::timestamp AT TIME ZONE 'Africa/Lagos')
  );
...
-- -----------------------------------------------------------------------------
-- C) VERIFY — duplicate groups on 11 Aug should be 0
-- -----------------------------------------------------------------------------
SELECT
  i.sale_id,
  i.product_id,
